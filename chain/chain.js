...
let blockstore = new FsBlockstore("./avatars/helia");
...
let wallet = engine.defaultInstance?.wallet;
let account = wallet[0].address;
...
app.post("/api/avatar", upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        // compare info to req.body
        if (data.avatar !== info.avatar) {
            console.log("Avatar didn't match chain");
            res.send("Avatar given didn't match chain");
            return;
        }
        ...
        //console.log(user)
        const file = new Uint8Array(req.file?.buffer);
        ...
        fs.writeFileSync("./avatars/" + info.avatar + ".png", req.file?.buffer);
